...
<head>
    <meta charset="utf-8">
    <title>{{ $quote->document_number }}</title>
    <style>
...
<body>
    @include('print.partials.document-header', [
        ...
        'meta' => [
            'Quote No' => $quote->document_number,
            'Date' => $quote->document_date->format('d M Y'),
            'Valid to' => $quote->expiry_date?->format('d M Y') ?? '—',
        ],
    ])

    <div class="block">
        <strong>For:</strong> {{ $quote->customer?->name ?? 'Cash Customer' }}
    ...
    <table class="totals">
        <tr><td>Subtotal (excl VAT)</td><td class="num">{{ number_format((float) $quote->subtotal_excl, 2) }}</td></tr>
        <tr><td>VAT</td><td class="num">{{ number_format((float) $quote->vat_amount, 2) }}</td></tr>
        <tr class="grand"><td>TOTAL</td><td class="num">{{ number_format((float) $quote->total_incl, 2) }}</td></tr>
    </table>

    @include('print.partials.document-footer', [
        'identity' => $identity,
        'footerText' => 'Prices are subject to stock availability and valid until the expiry date shown.',
    ])
</body>
</html>
